"""
AADA V2.5 - Adversarial AI Decision Analyzer (CLI)
====================================================
What's new in V2.5:
  - Four modes: Fast 2, Deep 2, Fast 3, Deep 3
  - GPT-4o integrated as a third critic (Fast 3, Deep 3)
  - All four modes share the same prompts.yaml config
  - Graceful failure when OPENAI_API_KEY is missing
  - JSON audit trail with consistent schema across all modes
  - Retry logic with exponential backoff on all three clients

Mode / Call Count:
  Fast 2  — 3 calls  (Claude → Gemini critique → Claude revision)
  Deep 2  — 5 calls  (two full Claude+Gemini passes)
  Fast 3  — 4 calls  (Claude → Gemini + GPT-4o critiques → Claude revision)
  Deep 3  — 7 calls  (two Claude+Gemini passes, then GPT-4o, then Claude final)
...
# Initialise clients — OpenAI is deferred until first use so missing key
# doesn't crash Fast 2 / Deep 2 runs.
...
def _null_stages() -> dict:
    """Return the canonical stage schema with all fields set to None."""
    return {
        "claude_initial":    None,
...
def run_deep2(user_query: str, prompts: dict, usage: dict) -> dict:
    """Deep 2 — 5 calls: Claude → Gemini → Claude → Gemini → Claude."""
    stages   = _null_stages()
    messages = [{"role": "user", "content": user_query}]
    ...
    _print_step(4, "Gemini critique (pass 2 — critiquing the revision)")
    gemini_critique_2 = call_gemini(
        prompts["critique_prompt"].format(user_query=user_query, claude_response=claude_revised),
        usage,
    )
    stages["gemini_critique_2"] = gemini_critique_2
    print(gemini_critique_2)

    _print_step(5, "Claude final revision (pass 2)")
...
def run_fast3(user_query: str, prompts: dict, usage: dict) -> dict:
    """Fast 3 — 4 calls: Claude → Gemini critique → GPT-4o critique → Claude revision."""
    stages   = _null_stages()
    messages = [{"role": "user", "content": user_query}]
    ...
    _print_step(3, "GPT-4o critique (receives Claude's answer + Gemini's critique)")
    openai_prompt = prompts["critique_prompt"].format(
        user_query=user_query, claude_response=claude_initial
...
def run_deep3(user_query: str, prompts: dict, usage: dict) -> dict:
    """Deep 3 — 7 calls: Claude → Gemini → Claude → Gemini → Claude → GPT-4o → Claude."""
    stages   = _null_stages()
    messages = [{"role": "user", "content": user_query}]
    ...
    _print_step(4, "Gemini critique (pass 2 — critiquing the revision)")
    gemini_critique_2 = call_gemini(
        prompts["critique_prompt"].format(user_query=user_query, claude_response=claude_revised_1),
        usage,
    )
    stages["gemini_critique_2"] = gemini_critique_2
    print(gemini_critique_2)
    ...
    _print_step(6, "GPT-4o critique (receives pass 2 revision + both Gemini critiques)")
    openai_prompt = (
        f"ORIGINAL QUESTION:\n{user_query}\n\n"
    ...
    final_revision_msg = (
        ...
        + f"\n\nA third AI (GPT-4o) has also reviewed your pass 2 answer and provided this critique:\n{openai_critique}\n\n"
...
def main():
    print("\n🔍 AADA V2.5 — Adversarial AI Decision Analyzer")
    print("─" * 50)
    ...
    # Validate OpenAI key early for 3-model modes
