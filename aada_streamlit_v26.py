"""
AADA V2.6 - Adversarial AI Decision Analyzer (Streamlit UI)
============================================================
What's new in V2.6:
  - Opt-in disagreement analysis checkbox (Fast 3 and Deep 3 only)
  - Analysis section renders open by default below the final answer
  - Four structured sections: Agreement, Disagreement, Reversals, Defended
  - JSON audit trail extended with disagreement_analysis field
...
def _set_status(slots: list, idx: int, state: str, label: str) -> None:
    slots[idx].markdown(f"{_STATUS_ICON.get(state, '')} **{label}** — *{state}*")

def _build_status_slots(step_labels: list) -> list:
    slots = []
...
def run_fast2(user_query, prompts, usage, status_slots, step_contents):
    stages   = _null_stages()
...
def run_deep2(user_query, prompts, usage, status_slots, step_contents):
    stages   = _null_stages()
    messages = [{"role": "user", "content": user_query}]
...
def run_fast3(user_query, prompts, usage, status_slots, step_contents):
    stages   = _null_stages()
    messages = [{"role": "user", "content": user_query}]
...
def run_deep3(user_query, prompts, usage, status_slots, step_contents):
    stages   = _null_stages()
    messages = [{"role": "user", "content": user_query}]
...
st.set_page_config(page_title="AADA V2.6", page_icon="🔍", layout="wide")
st.title("🔍 AADA V2.6 — Adversarial AI Decision Analyzer")
...
with st.sidebar:
    ...
    # Disagreement analysis checkbox — only shown for Fast 3 and Deep 3
...
if run_button:
    ...
    step_labels  = STEP_LABELS[mode_key]
    status_slots = _build_status_slots(step_labels)

    # If analysis is enabled, pre-build its waiting slot
    analysis_slot = None
    if run_analysis_call:
        analysis_slot = st.empty()
        analysis_slot.markdown("⏳ **Analysis — Claude — Disagreement Analysis** — *waiting*")
    ...
    if run_analysis_call and analysis_slot is not None:
        analysis_slot.markdown("🔄 **Analysis — Claude — Disagreement Analysis** — *running*")
        try:
            disagreement_analysis = run_analysis(stages, user_query, prompts, usage)
            analysis_slot.markdown("✅ **Analysis — Claude — Disagreement Analysis** — *complete*")
        except Exception as e:
            analysis_slot.markdown(f"❌ **Analysis failed:** {e}")
    ...
    # ── Disagreement analysis — open by default ──
    ...
    for label, content in step_contents[:-1]:
