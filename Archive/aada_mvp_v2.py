...
def call_claude(messages: list, usage: dict, system: str = None) -> str:
    """
    Send a conversation history to Claude with retry logic.
    Updates the shared usage dict with token counts after each successful call.
    """
...
def run_fast_pipeline(user_query: str, prompts: dict, usage: dict) -> dict:
    ...
    claude_initial = call_claude(messages, usage, system=prompts["system_prompt"])
    ...
    claude_final = call_claude(messages, usage)
...
def run_deep_pipeline(user_query: str, prompts: dict, usage: dict) -> dict:
    ...
    messages.append({"role": "user", "content": revision_message})
    claude_revised = call_claude(messages, usage)
...
def main():
    ...
    print(f"  Claude input tokens: {usage['claude_input_tokens']}")
    print(f"  Claude output tokens:{usage['claude_output_tokens']}")
    print(f"  Gemini input tokens: {usage['gemini_input_tokens']}")
    print(f"  Gemini output tokens:{usage['gemini_output_tokens']}")
